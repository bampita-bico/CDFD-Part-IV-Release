...
subatomic systems, and abstract or cognitive systems.
...
cosmic systems; plasma dynamics; nuclear dynamics; language evolution;
science paradigms; model diagnostics; falsification; reproducible research;
open science; preprint.

## GitHub Topics

`cdfd`, `cdfd-runtime`, `cdfl`, `adaptive-flux-limitation`,
`constraint-driven-flux-dynamics`, `systems-science`, `complex-systems`,
`network-science`, `earth-systems`, `climate-dynamics`, `ecology`,
`infrastructure`, `artificial-intelligence`,
`socioeconomic-systems`, `governance`, `epidemiology`, `immunology`,
`cosmic-systems`, `plasma-dynamics`, `reproducible-research`

## Contents

- `Part_A_Earth_Systems/` - Earth and environmental system manuscripts.
...
- `Part_E_Synthesis/outputs/partiv_discovery_summary.md` records the numbers that the papers cite.

These files record what the current runtime produces. They are useful for
checking the model and planning later measurement; they are not field evidence.

## Review Status

These manuscripts are research/preprint materials. Part IV uses CDFD/AFL
notation as a modelling language across many domains. A domain claim becomes
...
7. Each Part folder's `PDFs/` directory after compiling the manuscripts

## Public CDFD Runtime

The CDFD Runtime is the reusable implementation target for CDFL, the
flow-constraint-memory language used across the CDFD series. This Part IV
release is a scholarly archive: paper-local scripts import selected runtime
modules to reproduce diagnostics, and the archive remains cited and licensed
...
- Part II: https://doi.org/10.5281/zenodo.20264779
- Part III: https://doi.org/10.5281/zenodo.20344695
