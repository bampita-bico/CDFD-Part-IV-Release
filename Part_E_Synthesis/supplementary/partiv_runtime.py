...
RUNTIME_CANDIDATES = (
    WORKSPACE_ROOT / "CDFD-Runtime",
    WORKSPACE_ROOT / "cdfd_runtime",
)
RUNTIME_ROOT = next((path for path in RUNTIME_CANDIDATES if path.is_dir()), RUNTIME_CANDIDATES[0])
if not RUNTIME_ROOT.is_dir():
    raise RuntimeError(f"Could not locate the adjacent CDFD Runtime; checked {RUNTIME_CANDIDATES}")
...
def run_universal_network_cascade(
    ...
    for index in range(steps):
        ...
        # The current engine RK4 contract accepts scalar J_in. Inject the
        # declared localized field explicitly, then let the engine propagate it.
        state.phi += dt * source
        physics_step(state, dt=dt, J_in=0.0)
    ...
    payload = {
        ...
        "parameters": {
            ...
            "beta": beta,
            "threshold": threshold,
            "engine_update": "localized field injection followed by engine.physics.step RK4 with S and M_s dynamics",
        },
        "series": series,
        "summary": summary,
    }
...
def write_summary_markdown(runtime: dict[str, Any], cascade: dict[str, Any], sweep: dict[str, Any], figures: list[str]) -> None:
    ...
    run_date = cascade["provenance"]["timestamp_utc"].partition("T")[0]

    lines = [
        "# Part IV Runtime Discovery Summary",
        "",
        "This file is generated by `Part_E_Synthesis/supplementary/run_partiv_discovery.py`.",
        f"The numbers below are runtime output from the {run_date} model pass.",
        "",
        "## Runtime Surface",
        "",
